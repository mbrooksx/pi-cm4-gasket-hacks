				       ...
				       ulong location)
{
    ...
    u64 lower_fva, upper_fva;
    u32 level0_index,num_simple;
    lower_fva = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[0x48738]);
    upper_fva = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[0x48738 + 4]);
    u64 fva = (upper_fva << 32) | lower_fva;
    dev_info(gasket_dev->dev, "Fault VA: 0x%llx\n", fva);
    if (fva != -1 && fva) {
	 u64 extended_level0_index = (fva >> 21) & (8192 - 1);
	 u64 simple_level0_index = (fva >> 12) & (8192 - 1);
     const u32 num_simple_entries = 0x1800;
     u64 lower_bus_addr, upper_bus_addr;
     if (extended_level0_index < num_simple_entries) {
        dev_info(gasket_dev->dev, "Failing in first (simple) read access. Extended_level0: 0x%llx, Simple: 0x%llx\n", extended_level0_index, simple_level0_index);
        lower_bus_addr = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[0x50000 + 8*simple_level0_index]);
        upper_bus_addr = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[0x50000 + 8*simple_level0_index + 4]);
     } else {
        dev_info(gasket_dev->dev, "Failing in second read access. Extended_level0: 0x%llx, Simple: 0x%llx\n", extended_level0_index, simple_level0_index);
        lower_bus_addr = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[0x50000 + 8*extended_level0_index]);
        upper_bus_addr = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[0x50000 + 8*extended_level0_index + 4]);
     }
      dev_info(gasket_dev->dev, "Computed Failing Bus Addr: 0x%llx\n", (upper_bus_addr << 32) | lower_bus_addr);
    }
         

    u64 lower = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[location]);
    u64 upper = readl_relaxed(&gasket_dev->bar_data[bar].virt_base[location+4]);
    u64 result = (upper << 32) | lower;

    //dev_info(gasket_dev->dev, "location: 0x%lx lower: 0x%llx upper: 0x%llx "
    //                          "result: 0x%llx\n", location, lower, upper, result);
    return result;
}
...
// if (64)
    ...
    u32 upper = (value & 0xFFFFFFFF00000000) >> 32;
    writel_relaxed(lower, &dev->bar_data[bar].virt_base[location]);
    writel_relaxed(upper, &dev->bar_data[bar].virt_base[location+4]);
    //dev_info(dev->dev, "location: 0x%lx lower: 0x%lx upper: 0x%lx ",
