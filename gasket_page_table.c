				   ...
				   uint num_pages, u32 flags)
{
    ...
    dev_info(pg_tbl->device, "Map Simple Pages: host_addr 0x%lx, dev_addr 0x%llx, num_pages %d\n",
            host_addr, dev_addr, num_pages);
	if (ret) {
					  ...
					  u64 __iomem *slot)
{
    ...
    // dev_info(pg_tbl->device, "Subtable dma_addr: 0x%llx slot_id 0x%llx\n", dma_addr, slot - pg_tbl->base_slot);
    u32 lower = dma_addr & 0xFFFFFFFF;
    u32 upper = (dma_addr & 0xFFFFFFFF00000000) >> 32;
	writel(lower, slot);
	writel(upper, slot+4);
